//
//  AddTwoModeViewController.swift
//  Game_AddOne
//
...
class AddTwoModeViewController: UIViewController {
    ...
    func finishGame()
    {
        ...
        timer = nil
        if score > 8 {
            performSegue(withIdentifier: "TwoModeWin", sender: self)
        }
        else {
            performSegue(withIdentifier: "TwoModeLose", sender: self)
        }
        score = 0
        seconds = 60
        updateTimeLabel()
    ...
    override func prepare(for segue: UIStoryboaraadSegue, sender: Any?) {
