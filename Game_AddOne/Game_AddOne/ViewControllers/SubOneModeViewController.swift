//
//  SubOneModeViewController.swift
//  Game_AddOne
//
...
class SubOneModeViewController: UIViewController {
    ...
    func finishGame()
    {
        ...
        timer = nil
        if score > 8 {
            performSegue(withIdentifier: "SubModeWin", sender: self)
        }
        else {
            performSegue(withIdentifier: "SubModeLose", sender: self)
        }
        score = 0
        seconds = 60
        updateTimeLabel()
